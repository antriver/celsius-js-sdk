...
const HttpClient = function (config) {
  ...
  function _dispatchAxiosRequest (method, path, headers, data) {
    return axios({
      ...
      .then((response) => {
        if (response.headers['x-signature'] && !util.verifyResponse(response.data, response.headers['x-signature'])) {
      ...
      .catch((error) => {
        ...
        try {
          if (error.response) {
            if (error.response.headers['x-signature'] && !util.verifyResponse(error.response.data, error.response.headers['x-signature'])) {
              return Promise.reject(new CelsiusSDKError(ERROR_MESSAGES.RESPONSE_VERIFICATION_FAILED))
            }
